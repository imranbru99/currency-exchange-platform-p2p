...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalDeposit = Deposit::selectRaw('sum(amount * rate) as total_exchange')->where('user_id', $user->id)->first()->total_exchange;
        $reff = User::where('id',$user->ref_by)->first();
        $totalReferral = User::where('ref_by',$user->id)->count();
        $totalWithdraw = Withdrawal::where('user_id', $user->id)->where('status', 1)->selectRaw('sum(send_amount * rate) as Total')->first()->Total;
        $totalTransaction = Transaction::where('user_id', $user->id)->count();
        $refferal =  User::where('ref_by', $user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));

        $countPost = Post::where('status', '!=', 3)
        ...
        return view('admin.users.detail', compact('pageTitle', 'user','countries', 'countPost', 'countTicket', 'reff', 'totalReferral', 'totalDeposit', 'totalWithdraw', 'totalTransaction', 'refferal'));
    }
    ...
    public function deposits(Request $request, $id)
    {
        ...
        $scope = 'all';
        return view('admin.deposit.log', compact('pageTitle', 'user', 'deposits', 'empty_message','userId','scope','successful','pending','rejected'));
    }
    ...
    public function withdrawals(Request $request, $id)
    {
        ...
        $userId = $user->id;
        return view('admin.withdraw.withdrawals', compact('pageTitle', 'user', 'withdrawals', 'empty_message','userId'));
    }
    ...
    public function posts($id){
        $user = User::findOrFail($id);
        $pageTitle = 'All Posts of '.$user->fullname;
    ...
    public function tickets($id){
        $user = User::findOrFail($id);
        $pageTitle = 'Support Tickets of '.$user->username;
        $emptyMessage = 'No Data found.';
        $items = SupportTicket::where('user_id', $user->id)->with('user')->paginate(getPaginate());
